<?php
	...
	*/
		...
		private function lecturerToCompare($class_to_comp,$cur_class,$time){
			$buff = array();
			$lct = $this->_classes[$class_to_comp][$time]->getProf();	//Get the Supervisor at the intersection
			$lctGroup = $this->_classes[$cur_class][$time]->getProf();	//Get the Supervisor at the current time
			$buff[] = $lct;
			$buff[] = $lctGroup;
			return $buff;
		}		
